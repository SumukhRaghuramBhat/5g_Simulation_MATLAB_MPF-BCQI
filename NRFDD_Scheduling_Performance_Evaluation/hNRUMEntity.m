classdef hNRUMEntity < hNRRLCEntity
%hNRUMEntity Implement RLC UM functionality
...
% Copyright 2020 The MathWorks, Inc.
    ...
    properties (Access = public)
        ...
        %   38.321 Table 6.2.1-1. The default is 1.
        ...
        % SeqNumFieldLength Number of bits for sequence numbers of
        ...
        % ReassemblyTimer Timer for reassembly failure detection in ms
    ...
    methods (Access = public)
        function obj = hNRUMEntity(config)
            %hNRUMEntity Create an RLC UM entity
        ...
        function send(obj, rlcSDU)
            ...
            % buffer
            %   send(OBJ, RLCSDU) queues the received SDU in the Tx buffer.
            %   Also, it generates and stores the corresponding RLC UM
            %   header in a Tx header storage buffer.
            %
            ...
            % header in the RLC Tx header buffer. Thereafter, update the Tx
            % buffer occupancy length
            sduEnqueueIdx = mod(obj.TxBufferFront + obj.NumTxBufferSDUs, obj.MaxTxBufferSDUs) + 1;
            obj.TxBuffer{sduEnqueueIdx} = rlcSDU;
            obj.TxHeaderBuffer{sduEnqueueIdx} = obj.generateDataHeader(0, obj.TxSeqNumFieldLength, 0, 0);
        ...
        function rlcPDUSet = notifyTxOpportunity(obj, bytesGranted, remainingTBS)
            %notifyTxOpportunity Send the RLC protocol data units (PDUs)
            % that fit in the grant notified by MAC layer
            %   RLCPDUSET = notifyTxOpportunity(OBJ, BYTESGRANTED,
            %   REMAININGTBS) sends the RLC PDUs that fit in the grant
            %   notified by MAC.
            %
            %   RLCPDUSET is a cell array of RLC UMD PDUs.
            %
            ...
            while (numBytesFilled < bytesGranted) && ...
                ...
                pduLength = pduHeaderLength + length(obj.TxBuffer{obj.TxBufferFront + 1}(obj.SegmentOffset + 1:end));
                ...
                if ((numBytesFilled + pduLength + macHeaderLength) > bytesGranted) && ...
                    ...
                    if (minGrantLength >= remainingGrant)
                        % Send the updated RLC buffer status report to MAC layer
                        obj.TxBufferStatusFcn(obj.getBufferStatus());
                        return;
                    end
                    % Segment the SDU, and modify the header
                    if obj.SegmentOffset
                        % Create the RLC header for middle SDU segment
                        updatedHeader = obj.generateDataHeader(3, ...
                            obj.TxSeqNumFieldLength, obj.TxNext, obj.SegmentOffset);
                    else
                        % Create the RLC header for first SDU segment
                        updatedHeader = obj.generateDataHeader(1, ...
                            obj.TxSeqNumFieldLength, obj.TxNext, obj.SegmentOffset);
                    end
                    ...
                    segmentedPDU = [updatedHeader; ...
                        obj.TxBuffer{obj.TxBufferFront + 1}(obj.SegmentOffset + 1:obj.SegmentOffset + segmentedSDULength)];
                    ...
                    obj.SegmentOffset = obj.SegmentOffset + segmentedSDULength;
                    ...
                    newPDULength = length(obj.TxHeaderBuffer{obj.TxBufferFront + 1}) + ...
                        (length(obj.TxBuffer{obj.TxBufferFront + 1}) - obj.SegmentOffset);
                    ...
                    obj.RequiredGrantLength = obj.RequiredGrantLength - ...
                        ...
                        (macHeaderLength - obj.getMACHeaderLength(newPDULength));
                else
                    ...
                    normalPDU = [obj.TxHeaderBuffer{obj.TxBufferFront + 1}; ...
                        obj.TxBuffer{obj.TxBufferFront + 1}(obj.SegmentOffset + 1:end)];
                    ...
                    if (obj.SegmentOffset)
                        obj.TxNext = mod(obj.TxNext + 1, 2^(obj.TxSeqNumFieldLength));
                        obj.SegmentOffset = 0;
                    end
                    % Increment the utilized grant by the sum of RLC PDU
                    % length and MAC header length and decrement the
                    % required grant length by the sum of RLC PDU length
                    % and MAC header length
                    ...
                    obj.NumTxBufferSDUs = obj.NumTxBufferSDUs - 1;
                    % If Tx buffer is empty, update the required grant
                    % length to 0
        ...
        function bufferStatusReport = getBufferStatus(obj)
            ...
            bufferStatusReport = hNRRLCBufferStatus(obj.RNTI, obj.LogicalChannelID, ...
                obj.RequiredGrantLength);
        end

        function [sdu, sduLen] = receive(obj, packet)
            %receive Process the received PDU from the MAC layer
            %   [SDU, SDULEN] = receive(OBJ, PACKET) process the received
            %   PACKET from the MAC layer.
            %
            %   SDU is the output column vector of octets in decimal
            %   format.
            %
            ...
            obj.decodeDataPDU(packet, obj.RxSeqNumFieldLength);
            % Update the statistics
            obj.StatRxDataPDU = obj.StatRxDataPDU + 1;
            obj.StatRxDataBytes = obj.StatRxDataBytes + obj.DataPDUInfo.PDULength;
            % On the reception of complete SDU, forward it to upper layer
            if obj.DataPDUInfo.SegmentationInfo == 0
                sduLen = numel(obj.DataPDUInfo.Data);
                sdu = uint8(obj.DataPDUInfo.Data);
            else
                snModuloValue = obj.getModulusValue(obj.DataPDUInfo.SequenceNumber);
                % If the received SDU segment is a missing old segment,
                % discard it. Otherwise, store it in the reception buffer
                % for reassembly
                if(obj.getModulusValue(obj.RxNextHighest - obj.UMWindowSize) <= ...
                        snModuloValue) && (snModuloValue < obj.getModulusValue(obj.RxNextReassembly))
                    ...
                    % buffer
                    snBufIdx = obj.assignReassemblyBufIdx(obj.DataPDUInfo.SequenceNumber);
                    if snBufIdx < 0
                        [~, snBufIdx] = min(obj.getModulusValue(obj.ReassemblySNMap));
                        [numSegments, numBytes] = obj.RxBuffer{snBufIdx}.removeSNSegments();
                        ...
                        obj.ReassemblySNMap(snBufIdx) = obj.DataPDUInfo.SequenceNumber;
                    end
                    isLastSegment = false;
                    if obj.DataPDUInfo.SegmentationInfo == 2
                        isLastSegment = true;
                    end
                    [numDupBytes, isReassembled] = obj.RxBuffer{snBufIdx}.reassembleSegment(obj.DataPDUInfo.Data, obj.DataPDUInfo.PDULength, obj.DataPDUInfo.SegmentOffset, isLastSegment);
                    if numDupBytes
                    ...
                    if isReassembled
                        [rxSDU, sduLen] = obj.RxBuffer{snBufIdx}.getReassembledSDU();
                        sdu = rxSDU(1:sduLen);
                    end
                    % Update the RLC UM receiver state based on the
                    % received segmented SDU sequence number
                    obj.updateRxState(obj.DataPDUInfo.SequenceNumber, isReassembled);
                end
            end
        end
        ...
        function updateTimer(obj)
            ...
            if obj.ReassemblyTimeLeft > 0
                % Decrement the time left for reassembly timer trigger by 1
    ...
    methods (Access = private)
        ...
        function decodeDataPDU(obj, rlcPDU, seqNumFieldLength)
            ...
            if obj.DataPDUInfo.SegmentationInfo == 0
                ...
                if seqNumFieldLength == 6
                    ...
                    % received PDU
                    obj.DataPDUInfo.SequenceNumber = bitshift(bitand(rlcPDU(1), 15), 8) + rlcPDU(2);
                ...
                if obj.DataPDUInfo.SegmentationInfo == 1 % Check whether the first segment of the SDU is received
                    % Extract the RLC SDU segment from the first segmented
                    % RLC UMD PDU
                    obj.DataPDUInfo.SegmentOffset = 0;
                    obj.DataPDUInfo.Data = rlcPDU(segmentOffsetIndex:end);
                else
                    % Extract the RLC SDU segment offset information from
                    % the middle and last segmented RLC UMD PDUs
                    obj.DataPDUInfo.SegmentOffset = bitshift(rlcPDU(segmentOffsetIndex), 8) +  rlcPDU(segmentOffsetIndex+1);
                    % Extract the RLC SDU segment from the RLC UMD PDU
                    obj.DataPDUInfo.Data = rlcPDU(segmentOffsetIndex+2:end);
                end
            end
        end
        ...
        function updateRxState(obj, currPktSeqNum, isReassembled)
            ...
            if isReassembled
                ...
                if currPktSeqNum == obj.RxNextReassembly
                    minSn = currPktSeqNum + 1;
                    % Update the earliest SN considered for reassembly by
                    % finding a new sequence number which is not yet
                    % reassembled and delivered to upper layer
                    rnrModulus = obj.getModulusValue(obj.RxNextReassembly);
                    seqNumList = [minSn:obj.TotalRxSeqNum-1 0:minSn-2];
                    for snIdx = 1:numel(seqNumList)
                        if (obj.getModulusValue(seqNumList(snIdx)) > rnrModulus)
                            minSn = seqNumList(snIdx);
                        end
                    end
                    obj.RxNextReassembly = minSn;
                end
            elseif ~obj.isInsideReassemblyWindow(currPktSeqNum)
                obj.handleOutOfWindowSN(currPktSeqNum);
            end

            obj.updateRxStateOnRTRun();
            obj.updateRxStateOnRTStop();
        end
        ...
        function isInside = isInsideReassemblyWindow(obj, seqNum)
            ...
            % the flag to true
            if (obj.getModulusValue(obj.RxNextHighest - obj.UMWindowSize) <= ...
                    obj.getModulusValue(seqNum)) && (obj.getModulusValue(seqNum) < obj.getModulusValue(obj.RxNextHighest))
                isInside = true;
            end
        end
        ...
        function discardPDUsOutsideReassemblyWindow(obj)
            ...
            % PDUs which falls outside of the reassembly window

            % Iterate through the each SN and remove segmented PDUs
            ...
            for i = 1:obj.MaxReassemblySDU
                ...
                if obj.ReassemblySNMap(i) ~= -1
                    if ~obj.isInsideReassemblyWindow(obj.ReassemblySNMap(i))
                        % Discard the SDU segments which are received
                        % within the following sequence number and update
                        % the statistics
                        [numSegments, numBytes] = obj.RxBuffer{i}.removeSNSegments();
                        obj.StatRxDataPDUDropped = obj.StatRxDataPDUDropped + numSegments;
                        obj.StatRxDataBytesDropped = obj.StatRxDataBytesDropped + numBytes;
                    end
                end
        ...
        function reassemblyTimerExpiry(obj)
            ...
            minSn = obj.RxTimerTrigger;
            % Iterate through the missing SDU segments list
            seqNumList = [minSn:obj.TotalRxSeqNum-1 0:minSn-2];
            for snIdx = 1:numel(seqNumList)
                if (obj.getModulusValue(seqNumList(snIdx)) >= obj.getModulusValue(obj.RxTimerTrigger))
                    minSn = seqNumList(snIdx);
                end
            end
            ...
            for seqNumIdx = 1:obj.MaxReassemblySDU
                ...
                if (obj.getModulusValue(seqNum) < obj.getModulusValue(obj.RxNextReassembly))
                    % Discard the segmented PDUs which are received within
                    % the following sequence number and update the
                    % statistics
                    [numSegments, numBytes] = obj.RxBuffer{seqNumIdx}.removeSNSegments();
                    obj.StatRxDataPDUDropped = obj.StatRxDataPDUDropped + numSegments;
                    obj.StatRxDataBytesDropped = obj.StatRxDataBytesDropped + numBytes;
                end
            end
            ...
            rxNextHighest = obj.getModulusValue(obj.RxNextHighest);
            rnrBufIdx = obj.getSDUReassemblyBufIdx(obj.RxNextReassembly);
            if (rxNextHighest > obj.getModulusValue(rxNextReassemlby)) || ...
                    ((rxNextHighest == obj.getModulusValue(rxNextReassemlby)) && ...
                    ((rnrBufIdx ~= -1) && obj.RxBuffer(rnrBufIdx,1).anyLostSegment()))
                % Start the reassembly timer
                obj.ReassemblyTimeLeft = obj.ReassemblyTimer;
                obj.RxTimerTrigger = rxNextHighest;
            end
        end
        ...
        function updateRxStateOnRTRun(obj)
            ...
            if obj.ReassemblyTimeLeft > 0
                ...
                rxNextReassemlby = mod(obj.RxNextReassembly + 1, obj.TotalRxSeqNum);
                rnrBufIdx = obj.getSDUReassemblyBufIdx(obj.RxNextReassembly);
                if obj.getModulusValue(obj.RxTimerTrigger) <= obj.getModulusValue(obj.RxNextReassembly)
                    stopRT = true;
                elseif ~(obj.isInsideReassemblyWindow(obj.RxTimerTrigger)) && (obj.RxTimerTrigger ~= obj.RxNextHighest)
                    stopRT = true;
                elseif ((obj.RxNextHighest == rxNextReassemlby) &&  ...
                    ((rnrBufIdx ~= -1) && ~obj.RxBuffer{rnrBufIdx}.anyLostSegment()))
                    stopRT = true;
                end
        ...
        function updateRxStateOnRTStop(obj)
            ...
            if obj.ReassemblyTimeLeft == 0
                ...
                %   end + 1
                if obj.getModulusValue(obj.RxNextHighest) > obj.getModulusValue(rxNextReassembly)
                    startRT = true;
                elseif (obj.getModulusValue(obj.RxNextHighest) == obj.getModulusValue(rxNextReassembly)) && ...
                        ((rnrBufIdx ~= -1)&& obj.RxBuffer{rnrBufIdx}.anyLostSegment())
                    startRT = true;
                end
        ...
        function handleOutOfWindowSN(obj, currPktSeqNum)
            ...
            obj.RxNextHighest = mod(currPktSeqNum + 1, 2^obj.TotalRxSeqNum);
            % Discard the PDUs that fall outside of the reassembly window
            % due to window movement
            obj.discardPDUsOutsideReassemblyWindow();
            % Check whether the earliest SN considered for reassembly falls
            % outside of the reassembly window
            if ~obj.isInsideReassemblyWindow(obj.RxNextReassembly)
                minSn = currPktSeqNum + 1;
                % Update the earliest reassembly sequence number by finding
                % a new sequence number which is not yet reassembled and
                % delivered to higher layer as per Section 5.2.2.2.3 of the
                % 3GPP TS 38.322
                rnhUMDiffModulus = obj.getModulusValue(obj.RxNextHighest - obj.UMWindowSize);
                seqNumList = [minSn:obj.TotalRxSeqNum-1 0:minSn-2];
                for snIdx = 1:numel(seqNumList)
                    if (obj.getModulusValue(seqNumList(snIdx)) > rnhUMDiffModulus)
                        minSn = seqNumList(snIdx);
                    end
                end
                obj.RxNextReassembly = minSn;
