classdef hNRUEPassThroughPhy < hNRPhyInterface
    ...
    methods
        ...
        function run(obj)
            ...
            % It is assumed that MAC has alreay loaded the Phy Tx context for
        ...
        function advanceTimer(obj, numSym)
            ...
            if obj.CurrSymbol == 0 % Reached slot-boundary
                ...
                if obj.SlotsSinceChannelUpdate == obj.ChannelUpdatePeriodicity
                    ...
                    currPos = obj.GetPositionFcn();
                    disToGNB = norm(currPos - obj.GNBPosition);
        ...
        function dlTTIRequest(obj, pduType, dlTTIPDU)
            %dlTTIRequest Downlink transmission (non-data) time interval (TTI) request from MAC to Phy
            %   dlTTIRequest(OBJ, PDUTYPES, DLTTIPDUS) is a request
            ...
            %
            %   DLTTIPDU is an array of DL TTI PDUs corresponding to packet
            %   types in PDYTYPE. Currently supported CSI-RS PDU is an object
            %   of type nrCSIRSConfig.
            %   Pass-through does not send/receive actual CSI-RS. It
        ...
        function phyRx(obj)
            ...
            if ~isempty(csirsInfo)
                ...
                obj.CSIRSIndicationFcn(obj.ChannelQualityDL);
                obj.DataRxContext{symbolNumFrame + 1} = {}; % Clear the context
            end
        end
    ...
    methods (Access = private)
        ...
        function logPackets(obj, info, macPDU, linkDir)
            ...
            timestamp = round((obj.AFN*10 + (obj.CurrSlot*slotDuration) + ...
