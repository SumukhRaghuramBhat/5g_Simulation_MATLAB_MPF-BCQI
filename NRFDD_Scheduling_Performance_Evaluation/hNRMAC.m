classdef (Abstract) hNRMAC < handle
%hNRMAC Define an NR MAC base class
...
%   Copyright 2019-2020 The MathWorks, Inc.
    ...
    properties (Access = protected)
        ...
        RxDataRequestFcn
        %DlTTIRequestFcn Function handle to send DL-TTI request to Phy
        DlTTIRequestFcn
    ...
    properties (Constant)
        ...
        PDSCHFeedback = 4;
        %CQIReport Packet type for CQI report
        CQIReport = 5;
    ...
    methods (Access = public)
        ...
        function registerPhyInterfaceFcn(obj, txFcn, rxFcn, dlTTIReqFcn)
            %registerPhyInterfaceFcn Register Phy interface functions for Tx and Rx
            %   regiserPhyInterfaceFcn(OBJ, TXFCN, RXFCN, DLTTIREQFCN) registers Phy
            %   interface functions at MAC for (i) Sending packets to Phy
            %   and (ii) Sending Rx request to Phy at the Rx start time
            %   (iii) Sending DL-TTI request to Phy
            %
            %   TXFCN Function handle to send data to Phy.
            %
            %   RXFCN Function handle to indicate Rx start to Phy.
            %
            %   DLTTIREQFCN Function handle to send DL-TTI request to Phy.

            obj.TxDataRequestFcn = txFcn;
            obj.RxDataRequestFcn = rxFcn;
            obj.DlTTIRequestFcn = dlTTIReqFcn;
        end
        ...
        function [dataSubPDUList, remainingBytes] = lcpProcedure(obj, tbs, varargin)
            %lcpProcedure Perform the logical channel prioritization (LCP) procedure
            %
            %   [DATASUBPDULIST, REMAININGBYTES] = lcpProcedure(OBJ, TBS)
            %   performs the logical channel prioritization procedure in
            %   UE MAC.
            %
            %   [DATASUBPDULIST, REMAININGBYTES] = lcpProcedure(OBJ, TBS,
            %   RNTI) performs the logical channel prioritization
            %   procedure in gNB MAC for UE identified with RNTI.
            %
            ...
            for lchIdx = 1:numel(obj.LogicalChannelsConfig(ueIdx, :))
                if ~isempty(obj.LogicalChannelsConfig{ueIdx, lchIdx})
                    ...
                    if obj.LogicalChannelsConfig{ueIdx, lchIdx}.PBR ~= Inf
                        % Calculate the time elapsed since the last LCP run
                        % for the UE
                        timeElapsed = min(obj.ElapsedTimeSinceLastLCP(ueIdx, 1), obj.LogicalChannelsConfig{ueIdx, lchIdx}.BSD);
                        % Increment the Bj by the product of time elapsed
                        % and PBR
                        ...
                        % update the minimum grant Bj required by the
                        % logical channel to the minimum of logical channel
                        % buffer status and remaining grant
                        obj.LCHBjList(ueIdx, lchIdx) = min(obj.LCHBufferStatus(ueIdx, lchIdx), remainingBytes);
                    end
                    % Store the logical channel id in a cell array where
                    % the storing index of the cell array refer its
                    % priority
            ...
            % the LCP procedure. If only one logical channel has data to
            % transmit, then skip the LCP round-1. Otherwise, follow the
            % usual procedure
            ...
            % As per Section 5.4.3.1.3 of the 3GPP TS 38.321, perform the
            % LCP procedure
            for lcpRound = initialLCPRound:2
                % Iterate through the logical channel priority cell array,
                % from highest to lowest priority
                for priorityIdx = obj.MinPriorityForLCH:obj.MaxPriorityForLCH
                    % Check if it is LCP round-2
                    if (lcpRound == 2) && numel(lcpPriorityList{priorityIdx, 1})
                        % Share the grant equally among all logical
                        % channels with equal priority
                        avgGrant = obj.getEqualShareAmongLCH(lcpPriorityList{priorityIdx, 1}, remainingBytes, ueIdx);
                    end
                    % Iterate through the logical channels which are
                    % having the selected priority value
                    for j = 1:numel(lcpPriorityList{priorityIdx, 1})
                        ...
                        if (obj.LCHBufferStatus(ueIdx, lchIdx) ~= 0)
                            if lcpRound == 1
                                % In LCP round-1, don't consider logical
                                % channels whose Bj <= 0
                            ...
                            % Get the MAC SDUs from the RLC entity
                            macSDUs = obj.RLCTxFcn(ueIdx, lchIdx, grantSize, remainingBytes - grantSize);
                            utilizedGrant = 0;
                            % Calculate the number of bytes used by the
                            % logical channel
                            for sduIdx = 1:numel(macSDUs)
                                dataSubPDUList{dataPDUIdx} = hNRMACSubPDU(lchIdx, macSDUs{sduIdx}, obj.MACType);
                                utilizedGrant = utilizedGrant + numel(dataSubPDUList{dataPDUIdx});
                                dataPDUIdx = dataPDUIdx + 1;
                            end
                            if lcpRound == 1 && obj.LogicalChannelsConfig{ueIdx, lchIdx}.PBR ~= Inf
                                % In LCP round-1, decrement the Bj by the
                                % total number of bytes used by the logical
                                % channel
                                obj.LCHBjList(ueIdx, lchIdx) = obj.LCHBjList(ueIdx, lchIdx) - utilizedGrant;
                            end
                            % Update number of bytes left in the given TBS
                            remainingBytes = remainingBytes - utilizedGrant;
                            % As per Section 5.4.3.1.3 of the 3GPP TS
                            % 38.321, minimum required grant for a logical
                            % channel is 8 bytes
                            if remainingBytes < 8
        ...
        function pdu = constructMACPDU(obj, tbs, varargin)
            ...
            if obj.MACType
                % UE MAC
                [dataSubPDUList, remainingBytes] = obj.lcpProcedure(tbs);
            else
                % gNB MAC
                [dataSubPDUList, remainingBytes] = obj.lcpProcedure(tbs, varargin{1});
            end
            if remainingBytes > 0
                % Add padding subPDU for remaining bytes
                paddingSubPDU = hNRMACPaddingSubPDU(remainingBytes);
            end
            % Construct MAC PDU by concatenating subPDUs
            pdu = hNRMACMultiplex(dataSubPDUList, controlPDUList, paddingSubPDU, linkDir);
        end
